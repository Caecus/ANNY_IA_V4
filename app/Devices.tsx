...
export default function Devices() {
    ...
    // Estado para el switch de tipo de conexión
    const [connectionType, setConnectionType] = useState<'wifi' | 'bluetooth'>('wifi');
    // Estado para evitar doble desconexión
    ...
    // Lógica para alternar entre Wi-Fi y Bluetooth
    const handleSwitch = async (value: boolean) => {
        if (switching) return;
        setSwitching(true);
        // Si cambiamos a Bluetooth, desconectar Wi-Fi si está conectado
        if (value && connected.wifi) {
            try {
                await glasses.glassesDisconnect?.();
                dispatch(disconnectDevice('wifi'));
            } catch {}
        }
        // Si cambiamos a Wi-Fi, desconectar Bluetooth si está conectado
        if (!value && connected.bluetooth) {
            try {
                await glasses.glassesDisconnect?.();
                dispatch(disconnectDevice('bluetooth'));
            } catch {}
        }
        setConnectionType(value ? 'bluetooth' : 'wifi');
    ...
    return (
        <View style={styles.container}>
            ...
            {/* Switch para alternar tipo de conexión */}
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
                <Text style={{ marginRight: 8, color: connectionType === 'wifi' ? colors.primary : '#888', fontWeight: connectionType === 'wifi' ? 'bold' : 'normal' }}>Wi-Fi</Text>
                <Switch
                    value={connectionType === 'bluetooth'}
                    onValueChange={handleSwitch}
                    thumbColor={connectionType === 'bluetooth' ? colors.primary : '#ccc'}
                    trackColor={{ false: '#ccc', true: colors.primary }}
                    disabled={switching}
                />
                <Text style={{ marginLeft: 8, color: connectionType === 'bluetooth' ? colors.primary : '#888', fontWeight: connectionType === 'bluetooth' ? 'bold' : 'normal' }}>Bluetooth</Text>
            </View>
